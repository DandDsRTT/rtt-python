...
def render_lines(r, layout, seen) -> None:
    ...
    def place_line(line, suffix, parent, pane="body"):
        shift = 0 if pane == "col" else freeze_y
    ...
    for line in layout.lines:
        ...
        if x1 >= freeze_x and y1 >= freeze_y:
            place_line(line, "", r._chrome.board)
            if line.orientation == "v" and y0 <= freeze_y and line.length > freeze_y:
                place_line(line, "#fill", r._chrome.columnfill_inner)
        if x1 >= freeze_x and y0 < freeze_y:
            place_line(line, "#col", r._chrome.columnhead_inner, "col")
        if x0 < freeze_x and y1 >= freeze_y:
            place_line(line, "#row", r._chrome.rowband, "row")
...
def render_cells(r, layout, seen, flags) -> None:
    ...
    freeze_x, freeze_y = layout.freeze_x, layout.freeze_y
    paint = (freeze_y, structural and not cold, rings, layout.preview_hold, layout.preview_offset)
