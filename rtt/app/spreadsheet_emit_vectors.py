...
class _EmitVectorsMixin:
    ...
    def _emit_vectors_band(self) -> None:
        ...
        if self.tile_open("vectors", "targets"):
            ...
            self._emit_vec_grid(_VecGrid("targets", _r.dims.k, ids.target_cell, self.target_left,
                cell_inset, target_kind, "targetcell", _r.targets.vectors, _r.targets.pending, _r.tuning.target_sizes))
        if self.tile_open("vectors", "held"):
            self._emit_vec_grid(_VecGrid("held", _r.dims.nh, ids.held_cell, self.held_left,
                0, "heldcell", "heldcell", _r.held.vectors, _r.held.pending, _r.tuning.held_sizes))
        if self.tile_open("vectors", "detempering"):
            self._emit_vectors_detempering_col()
        if self.tile_open("vectors", "interest"):
            self._emit_vec_grid(_VecGrid("interest", _r.dims.mi, ids.interest_cell, self.interest_left,
                KET_INSET, "interestcell", "interestcell", _r.interest.vectors, _r.interest.pending, _r.tuning.interest_sizes))
    ...
    def _emit_vectors_basis_col(self) -> None:
        ...
        if _r.scalars.element_draft:
            ...
            self.cells.append(CellBox("basis:pending", bx, self.vec_top(_r.dims.d), COL_W, ROW_H,
                                      self._element_cell_kind(draft_text), text=draft_text, prime=_r.dims.d, pending=True))
            self._emit_basis_minus("element_minus:basis:pending", _r.dims.d, "element_minus")
        if _r.flags.nonstandard_domain:
            ...
            self._emit_basis_minus("basis_minus", _r.dims.d - 1, "basis_minus")
    ...
    def _emit_vectors_commas_col(self) -> None:
        ...
        for c in range(_r.dims.nc):
            for p in range(_r.dims.d):
                ...
                self._voice("vectors:commas", c, _r.tuning.comma_sizes.just[c])
            if _r.flags.presets:
        ...
        for j in range(_r.dims.nu):
            ...
            for p in range(_r.dims.d):
                ...
                self.cells.append(CellBox(ids.unchanged_cell(j, p), self.comma_left(_r.dims.nc_shown + j), self.vec_top(p), COL_W, ROW_H,
            ...
            self._voice("vectors:commas", _r.dims.nc + j, _r.unchanged.sizes.just[j])
        if _r.scalars.comma_draft:
            ...
            for p in range(_r.dims.d):
                ...
                self.cells.append(CellBox(ids.comma_cell(self.pending_col_token('commas'), p), self.comma_left(_r.dims.nc), self.vec_top(p), COL_W, ROW_H, col_kind,
                                     text="" if v is None else str(v), prime=p, comma=_r.dims.nc, pending=True, unit=self.cell_unit("vectors", "commas", prime=p)))
            if _r.commas.pending is not None and _r.flags.presets:
                self.cells.append(CellBox("commapick:draft", self.comma_left(_r.dims.nc), self.cpick_band_y("vectors") + COMMAPICK_GAP, COL_W, ROW_H, "commapick", comma=_r.dims.nc, pending=True))

    def _emit_vectors_detempering_col(self) -> None:
        _r = self.resolved
        for i in range(_r.dims.r):
            for p in range(_r.dims.d):
                self.cells.append(CellBox(f"cell:vec:detempering:{self.col_token('detempering', i)}:{p}", self.detempering_left(i), self.vec_top(p), COL_W, ROW_H, "vec", text=str(_r.detempering.vectors[i][p]), unit=self.cell_unit("vectors", "detempering", prime=p)))
                self._voice("vectors:detempering", i, _r.detempering.sizes.just[i])
    ...
    def _emit_vectors_int_handles(self) -> None:
        ...
        if "vectors" in self.rows and self.rows["vectors"].int_handle_top is not None:
            ...
            for group, count, col_left, ckey in (("comma", _r.dims.nc, self.comma_left, "commas"),
                                                 ("target", _r.dims.k, self.target_left, "targets"),
                                                 ("held", _r.dims.nh, self.held_left, "held"),
                                                 ("interest", _r.dims.mi, self.interest_left, "interest")):
                if count >= 2 and self.tile_open("vectors", ckey) and (ckey != "targets" or _r.scalars.targets_editable):
