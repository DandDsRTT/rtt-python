...
def _emit_mapping_gens(cells, resolved, geometry, context) -> None:
    ...
    for i in range(resolved.dims.rank):
        cells.append(CellBox(f"gen:{query.column_token(resolved, 'gens', i)}", geometry.column_x["quantities"], query.map_top(geometry, i), geometry.column_width["quantities"], ROW_H, "genratio", text=resolved.scalars.gens[i] if i < len(resolved.scalars.gens) else "", gen=i))
    ...
    if resolved.dims.rank > 1:
        for i in range(resolved.dims.rank):
            cells.append(CellBox(f"map_minus:{query.column_token(resolved, 'gens', i)}", map_bus_x, query.map_top(geometry, i), gen_right - map_bus_x, ROW_H, "map_minus", gen=i))
...
def _emit_mapping_drag(cells, resolved, geometry, context) -> None:
    if context.settings.get("drag_to_combine") and resolved.dims.rank > 1 and query.tile_open(geometry, context.collapsed, "mapping", "primes"):
        for i in range(resolved.dims.rank):
            cells.append(CellBox(f"map_drag:{query.column_token(resolved, 'gens', i)}", geometry.primes_x + query.etpick_left_pad(geometry, "primes"), query.map_top(geometry, i), ROW_HANDLE_W, ROW_H, "map_drag", gen=i))
    ...
    for i in range(resolved.dims.rank):
        ...
        if query.tile_open(geometry, context.collapsed, "mapping", "primes"):
            ...
            for p in range(resolved.dims.dimensionality):
                cells.append(CellBox(ids.mapping_cell(rt, p), query.prime_left(geometry, p), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapping", text=str(context.state.mapping[i][p]), gen=i, prime=p, unit=query.cell_unit(resolved, "mapping", "primes", gen=i, prime=p)))
...
def _emit_mapping_comma_row(cells, resolved, geometry, i, rt) -> None:
    for c in range(resolved.dims.comma_count):
        cells.append(CellBox(f"cell:mapped_comma:{rt}:{query.column_token(resolved, 'commas', c)}", query.comma_left(geometry, resolved, c), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(resolved.commas.mapped[i][c]), gen=i, unit=query.cell_unit(resolved, "mapping", "commas", gen=i)))
    ...
    if resolved.scalars.comma_draft:
        ...
        cells.append(CellBox(f"cell:mapped_comma:{rt}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dims.comma_count), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=mc_text, gen=i, pending=True))
    for j in range(resolved.dims.unchanged_count):
        ...
        cells.append(CellBox(f"cell:mapped_unchanged:{rt}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=mapped_text, gen=i, unit=query.cell_unit(resolved, "mapping", "commas", gen=i)))
...
def _emit_mapping_draft_row(cells, resolved, geometry, context) -> None:
    ...
    if query.tile_open(geometry, context.collapsed, "mapping", "quantities"):
        ...
        cells.append(CellBox("gen:pending", geometry.column_x["quantities"], query.map_top(geometry, dr), geometry.column_width["quantities"], ROW_H, "genratio", text=gen_text, gen=dr, pending=True))
    ...
    if query.tile_open(geometry, context.collapsed, "mapping", "primes"):
        ...
        for p in range(resolved.dims.dimensionality):
            ...
            cells.append(CellBox(ids.mapping_cell(drt, p), query.prime_left(geometry, p), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, row_kind, text="" if v is None else str(v), gen=dr, prime=p, pending=True))
...
def _emit_mapping_draft_mapped(cells, resolved, geometry, context, dr, drt) -> None:
    if query.tile_open(geometry, context.collapsed, "mapping", "targets"):
        for j in range(resolved.dims.target_count):
            cells.append(CellBox(f"cell:mapped:{drt}:{query.column_token(resolved, 'targets', j)}", query.target_left(geometry, j), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, "mapped", text=_draft_mapped_text(resolved, "targets", j), gen=dr, pending=True))
    if query.tile_open(geometry, context.collapsed, "mapping", "interest"):
        for ii in range(resolved.dims.interest_count):
            cells.append(CellBox(f"cell:imapped:{drt}:{query.column_token(resolved, 'interest', ii)}", query.interest_left(geometry, ii), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, "mapped", text=_draft_mapped_text(resolved, "interest", ii), gen=dr, pending=True))
    if query.tile_open(geometry, context.collapsed, "mapping", "held"):
        for hi in range(resolved.dims.held_count):
            cells.append(CellBox(f"cell:hmapped:{drt}:{query.column_token(resolved, 'held', hi)}", query.held_left(geometry, hi), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, "mapped", text=_draft_mapped_text(resolved, "held", hi), gen=dr, pending=True))
    if query.tile_open(geometry, context.collapsed, "mapping", "commas"):
...
def _emit_mapping_draft_commas(cells, resolved, geometry, dr, drt) -> None:
    for c in range(resolved.dims.comma_count):
        cells.append(CellBox(f"cell:mapped_comma:{drt}:{query.column_token(resolved, 'commas', c)}", query.comma_left(geometry, resolved, c), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, "mapped", text=_draft_mapped_text(resolved, "commas", c), gen=dr, pending=True))
    for j in range(resolved.dims.unchanged_count):
        cells.append(CellBox(f"cell:mapped_unchanged:{drt}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_H, "mapped", text=_draft_mapped_text(resolved, "unchanged", j), gen=dr, pending=True))


def _emit_mapped_tile(cells, resolved, geometry, m: _MappedTile, i, rt) -> None:
    for col in range(m.count):
        cells.append(CellBox(f"cell:{m.prefix}:{rt}:{query.column_token(resolved, m.group, col)}", m.left_fn(col), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(m.data[i][col]), gen=i, unit=query.cell_unit(resolved, "mapping", m.group, gen=i)))
        if m.sizes is not None:
            voice(cells, f"mapped:{m.group}", col, m.sizes[col])
    if m.pending is not None:
        cells.append(CellBox(f"cell:{m.prefix}:{rt}:draft", m.left_fn(m.count), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text="", gen=i, pending=True))
    ...
    for j in range(n_cols):
        for i in range(height):
            ...
            cells.append(CellBox(f"cell:{prefix}:{token}:{i}", left(j), top(i),
                                 COLUMN_WIDTH, ROW_H, "mapped", text=text, prime=i, **{column_kw: j}))
            if sizes is not None:
                voice(cells, audio, j, sizes[j])
    if pending is not None:
        for i in range(height):
            cells.append(CellBox(f"cell:{prefix}:draft:{i}", left(n_cols), top(i),
                                 COLUMN_WIDTH, ROW_H, "mapped", text="", prime=i, pending=True))
    ...
    for i in range(height):
        for j in range(n_cols):
            ...
            cells.append(CellBox(f"cell:{prefix}:{i}:{j}", left(j), top(i),
                                 COLUMN_WIDTH, ROW_H, "mapped", text=text, **{column_kw: j}))
    ...
    if resolved.scalars.comma_draft:
        for p in range(resolved.dims.dimensionality):
            cells.append(CellBox(f"cell:projection_vectors:{p}:draft", query.comma_left(geometry, resolved, resolved.dims.comma_count), query.projection_top(geometry, p),
                                 COLUMN_WIDTH, ROW_H, "mapped", text="0" if resolved.ghosts.comma else "", prime=p, pending=True))
    for j in range(resolved.dims.unchanged_count):
...
def _emit_projection_basis(cells, resolved, geometry, context) -> None:
    if query.row_open(geometry, context.collapsed, "projection") and query.tile_open(geometry, context.collapsed, "projection", "quantities"):
        ...
        for p in range(resolved.dims.dimensionality):
            cells.append(CellBox(f"projection_basis:{p}", bx, query.projection_top(geometry, p), COLUMN_WIDTH, ROW_H, "commaratio", text=str(resolved.dims.elements[p]), prime=p))
    ...
    if query.row_open(geometry, context.collapsed, "scaling_factors") and query.tile_open(geometry, context.collapsed, "scaling_factors", "commas"):
        ...
        for c, lam in enumerate(scaling):
            cells.append(CellBox(f"cell:scaling:{query.column_token(resolved, 'commas', c)}", query.comma_left(geometry, resolved, query.comma_value_pos(resolved, c)), geometry.rows["scaling_factors"].y,
                                 COLUMN_WIDTH, ROW_H, "mapped", text=lam, comma=c))
        if resolved.scalars.comma_draft:
            cells.append(CellBox("cell:scaling:draft", query.comma_left(geometry, resolved, resolved.dims.comma_count), geometry.rows["scaling_factors"].y,
                                 COLUMN_WIDTH, ROW_H, "mapped", text="0" if resolved.ghosts.comma else "", pending=True))
...
def _emit_canon_gens(cells, resolved, geometry, context) -> None:
    if query.tile_open(geometry, context.collapsed, "canon", "quantities"):
        for i in range(resolved.dims.canonical_rank):
            cells.append(CellBox(f"canon:gen:{i}", geometry.column_x["quantities"], query.canon_top(geometry, i), geometry.column_width["quantities"], ROW_H, "genratio", text=resolved.canon.gens[i] if i < len(resolved.canon.gens) else ""))
...
def _emit_canon_primes(cells, resolved, geometry, context) -> None:
    if query.tile_open(geometry, context.collapsed, "canon", "primes"):
        for i in range(resolved.dims.canonical_rank):
            for p in range(resolved.dims.dimensionality):
                cells.append(CellBox(f"cell:canon:{i}:{p}", query.prime_left(geometry, p), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(resolved.canon.mapping[i][p]), gen=i, prime=p, unit=query.cell_unit(resolved, "canon", "primes", gen=i, prime=p)))
    ...
    if query.tile_open(geometry, context.collapsed, "canon", "gens"):
        for i in range(len(resolved.canon.form_M)):
            for j in range(len(resolved.canon.form_M)):
                cells.append(CellBox(f"cell:form:{i}:{j}", query.gen_left(geometry, j), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(resolved.canon.form_M[i][j]), unit=query.cell_unit(resolved, "canon", "gens", gen=i)))
...
def _emit_canon_row(cells, resolved, geometry, context, i) -> None:
    ...
    if query.tile_open(geometry, collapsed, "canon", "detempering"):
        for c in range(resolved.dims.rank):
            cells.append(CellBox(f"cell:canon_detempering:{i}:{query.column_token(resolved, 'detempering', c)}", query.detempering_left(geometry, c), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(resolved.canon.mapped_detempering[i][c]), gen=i, unit=query.cell_unit(resolved, "canon", "detempering", gen=i)))
...
def _emit_canon_comma_row(cells, resolved, geometry, i) -> None:
    for c in range(resolved.dims.comma_count):
        cells.append(CellBox(f"cell:canon_mapped_comma:{i}:{query.column_token(resolved, 'commas', c)}", query.comma_left(geometry, resolved, c), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(resolved.canon.mapped_commas[i][c]), gen=i, unit=query.cell_unit(resolved, "canon", "commas", gen=i)))
    if resolved.scalars.comma_draft:
        cells.append(CellBox(f"cell:canon_mapped_comma:{i}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dims.comma_count), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text="", gen=i, pending=True))
    for j in range(resolved.dims.unchanged_count):
        ut = DASH if resolved.canon.unchanged_mapped[i][j] is None else str(resolved.canon.unchanged_mapped[i][j])
        cells.append(CellBox(f"cell:canon_mapped_unchanged:{i}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=ut, gen=i, unit=query.cell_unit(resolved, "canon", "commas", gen=i)))
    ...
    if query.tile_open(geometry, context.collapsed, "mapping", "canongens"):
        for i in range(resolved.dims.rank):
            for j in range(resolved.dims.canonical_rank):
                cells.append(CellBox(f"cell:finv:{i}:{j}", query.canongen_left(geometry, j), query.map_top(geometry, i), COLUMN_WIDTH, ROW_H,
...
def _emit_canon_mapped_tile(cells, resolved, geometry, prefix, group, count, left_fn, data, pending, i) -> None:
    for col in range(count):
        cells.append(CellBox(f"cell:{prefix}:{i}:{query.column_token(resolved, group, col)}", left_fn(col), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text=str(data[i][col]), gen=i, unit=query.cell_unit(resolved, "canon", group, gen=i)))
    if pending is not None:
        cells.append(CellBox(f"cell:{prefix}:{i}:draft", left_fn(count), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_H, "mapped", text="", gen=i, pending=True))
