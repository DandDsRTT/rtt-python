    ...
    for i in range(resolved.dimensions.rank):
        ...
        if query.tile_open(geometry, context.collapsed, "mapping", "primes"):
            ...
            for p in range(resolved.dimensions.dimensionality):
                cells.append(Cell(ids.mapping_cell(rt, p), query.prime_left(geometry, p), bands.map_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapping", text=str(context.state.mapping[i][p]), generator=i, prime=p, unit=query.cell_unit(resolved, "mapping", "primes", generator=i, prime=p)))
...
def _emit_mapping_comma_row(cells, resolved, geometry, i, rt) -> None:
    ...
    if resolved.scalars.comma_draft:
        cells.append(Cell(f"cell:mapped_comma:{rt}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dimensions.comma_count), bands.map_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text="", generator=i, pending=True))
...
def _emit_mapping_draft_row(cells, resolved, geometry, context) -> None:
    ...
    if query.tile_open(geometry, context.collapsed, "mapping", "quantities"):
        ...
        cells.append(Cell("generator:pending", query.basis_col_x(geometry), bands.map_top(geometry, dr), COLUMN_WIDTH, ROW_HEIGHT, "generator_ratio", text=generator_text, generator=dr, pending=True))
...
def emit_projection_band(resolved, geometry, context) -> EmitResult:
    ...
    emit_mapped_grid(cells, resolved, geometry, collapsed, "generator_embedding", "embed", resolved.projection.embedding_matrix, resolved.dimensions.rank, lambda i: query.generator_embedding_left(geometry, i), "generator")
    emit_mapped_grid(cells, resolved, geometry, collapsed, "canonical_generators", "embed_c", resolved.canonical.detempering, resolved.dimensions.canonical_rank, lambda i: query.canonical_generator_left(geometry, i), "generator")
    ...
    emit_mapped_grid(cells, resolved, geometry, collapsed, "generators", "projection_detempering", resolved.projection.detempering, resolved.dimensions.rank, lambda i: query.detempering_left(geometry, i), "generator",
                     full=full_projection, colwise=True, column_token_key="generators", audio="projection:detempering")
...
def _emit_projection_basis(cells, resolved, geometry, context) -> None:
    if query.row_open(geometry, context.collapsed, "projection") and query.tile_open(geometry, context.collapsed, "projection", "quantities"):
        ...
        for p in range(resolved.dimensions.dimensionality):
            cells.append(Cell(f"projection_basis:{p}", bx, bands.projection_top(geometry, p), COLUMN_WIDTH, ROW_HEIGHT, "comma_ratio", text=str(resolved.dimensions.elements[p]), prime=p))
...
def _emit_canonical_generators(cells, resolved, geometry, context) -> None:
    if query.tile_open(geometry, context.collapsed, "canonical", "quantities"):
        for i in range(resolved.dimensions.canonical_rank):
            cells.append(Cell(f"canonical:generator:{i}", query.basis_col_x(geometry), bands.canonical_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "generator_ratio", text=resolved.canonical.generators[i] if i < len(resolved.canonical.generators) else ""))
...
def _emit_canonical_primes(cells, resolved, geometry, context) -> None:
    if query.tile_open(geometry, context.collapsed, "canonical", "primes"):
        for i in range(resolved.dimensions.canonical_rank):
            for p in range(resolved.dimensions.dimensionality):
                cells.append(Cell(f"cell:canonical:{i}:{p}", query.prime_left(geometry, p), bands.canonical_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=str(resolved.canonical.mapping[i][p]), generator=i, prime=p, unit=query.cell_unit(resolved, "canonical", "primes", generator=i, prime=p)))
    ...
    if query.tile_open(geometry, context.collapsed, "canonical", "generators"):
        for i in range(len(resolved.canonical.inverse_form_M)):
            for j in range(len(resolved.canonical.inverse_form_M)):
                cells.append(Cell(f"cell:inverse_form:{i}:{j}", query.generator_left(geometry, j), bands.canonical_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=str(resolved.canonical.inverse_form_M[i][j]), unit=query.cell_unit(resolved, "canonical", "generators", generator=i)))
...
def _emit_canonical_row(cells, resolved, geometry, context, i) -> None:
    ...
    if query.tile_open(geometry, collapsed, "canonical", "detempering"):
        for c in range(resolved.dimensions.rank):
            cells.append(Cell(f"cell:canonical_detempering:{i}:{query.column_token(resolved, 'detempering', c)}", query.detempering_left(geometry, c), bands.canonical_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=str(resolved.canonical.mapped_detempering[i][c]), generator=i, unit=query.cell_unit(resolved, "canonical", "detempering", generator=i)))
...
def _emit_canonical_comma_row(cells, resolved, geometry, i) -> None:
    ...
    if resolved.scalars.comma_draft:
        cells.append(Cell(f"cell:canonical_mapped_comma:{i}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dimensions.comma_count), bands.canonical_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text="", generator=i, pending=True))
    for j in range(resolved.dimensions.unchanged_count):
