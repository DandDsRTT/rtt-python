...
def _emit_mapping_comma_row(cells, resolved, geometry, i, rt) -> None:
    ...
    if resolved.scalars.comma_draft:
        ...
        cells.append(CellBox(f"cell:mapped_comma:{rt}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dims.comma_count), query.map_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=mc_text, gen=i, pending=True))
    for j in range(resolved.dims.unchanged_count):
        ...
        cells.append(CellBox(f"cell:mapped_unchanged:{rt}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.map_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=mapped_text, gen=i, unit=query.cell_unit(resolved, "mapping", "commas", gen=i)))
...
def _emit_mapping_draft_commas(cells, resolved, geometry, dr, drt) -> None:
    ...
    for j in range(resolved.dims.unchanged_count):
        cells.append(CellBox(f"cell:mapped_unchanged:{drt}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.map_top(geometry, dr), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=_draft_mapped_text(resolved, "unchanged", j), gen=dr, pending=True))
...
def _emit_projection_basis(cells, resolved, geometry, context) -> None:
    if query.row_open(geometry, context.collapsed, "projection") and query.tile_open(geometry, context.collapsed, "projection", "quantities"):
        ...
        for p in range(resolved.dims.dimensionality):
            cells.append(CellBox(f"projection_basis:{p}", bx, query.projection_top(geometry, p), COLUMN_WIDTH, ROW_HEIGHT, "commaratio", text=str(resolved.dims.elements[p]), prime=p))
...
def _emit_canon_comma_row(cells, resolved, geometry, i) -> None:
    ...
    if resolved.scalars.comma_draft:
        cells.append(CellBox(f"cell:canon_mapped_comma:{i}:{query.pending_col_token(resolved, 'commas')}", query.comma_left(geometry, resolved, resolved.dims.comma_count), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text="", gen=i, pending=True))
    for j in range(resolved.dims.unchanged_count):
        ut = DASH if resolved.canon.unchanged_mapped[i][j] is None else str(resolved.canon.unchanged_mapped[i][j])
        cells.append(CellBox(f"cell:canon_mapped_unchanged:{i}:{j}", query.comma_left(geometry, resolved, resolved.dims.comma_count_shown + j), query.canon_top(geometry, i), COLUMN_WIDTH, ROW_HEIGHT, "mapped", text=ut, gen=i, unit=query.cell_unit(resolved, "canon", "commas", gen=i)))
