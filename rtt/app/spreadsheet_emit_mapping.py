...
def _emit_mapping_drag(cells, resolved, geometry, ctx) -> None:
    if ctx.settings.get("drag_to_combine") and resolved.dims.r > 1 and query.tile_open(geometry, ctx.collapsed, "mapping", "primes"):
        for i in range(resolved.dims.r):
    ...
    for i in range(resolved.dims.r):
        ...
        if query.tile_open(geometry, ctx.collapsed, "mapping", "primes"):
            ...
            for p in range(resolved.dims.d):
                cells.append(CellBox(ids.mapping_cell(rt, p), query.prime_left(geometry, p), query.map_top(geometry, i), COL_W, ROW_H, "mapping", text=str(ctx.state.mapping[i][p]), gen=i, prime=p, unit=query.cell_unit(resolved, "mapping", "primes", gen=i, prime=p)))
...
def _emit_mapping_draft_row(cells, resolved, geometry, ctx) -> None:
    ...
    if query.tile_open(geometry, ctx.collapsed, "mapping", "primes"):
        ...
        for p in range(resolved.dims.d):
            v = resolved.ghosts.row_map[p] if resolved.ghosts.row else ctx.pending_mapping_row[p]
...
def _emit_projection_unchanged(cells, resolved, geometry, ctx) -> None:
    if not (resolved.unchanged.shown and query.row_open(geometry, ctx.collapsed, "projection")
