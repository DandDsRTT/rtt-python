...
# The zoom-on-hover magnifier (app._add_zoom_tooltip). Hovering a read-only gridded value pops a
# tooltip showing that SAME value face enlarged — scaled so a small cents whole-part (the
# .rtt-stacked-main 10px of "1200.000") reaches the normal integer cell size (--cell-font). The
# markup these builders emit mirrors the live cell's classes EXACTLY (rtt-tuning-value /
# rtt-stacked-* / rtt-ratio / rtt-frac / rtt-value / rtt-mathexpr-stack), so the .rtt-zoom-scale CSS
# transform scales an identical face (a literal zoom, not a re-styled one) and the light/dark value
# cascades retint it for free. The kinds covered are the read-only value faces that can render below
# the integer size — the ones a magnifier is for; the plain integer matrix cells already sit at that
# reference size, and the editable cells keep their how-to-edit help tooltip.
ZOOM_KINDS = frozenset({"tuningvalue", "genratio", "commaratio", "powerdisplay", "mathexpr"})


def zoom_blank(text: str) -> bool:
    """Whether a zoom face would carry no real value — quantities off (blank), or a dashed /
    placeholder entry ("—", "—/—", "?/?") — so :func:`zoom_tip_html` suppresses the magnifier
    rather than popping an empty or all-dashes bubble. A value with any digit (so "1200.000" or a
    bare "0") is never blank."""
    stripped = (text or "").strip()
    return not stripped or set(stripped) <= set("—-/?. ")


def _stacked_face_html(main: str, sub: str) -> str:
    """The stacked main-over-sub face as HTML, mirroring app._put_stacked_face / _size_stacked_main:
    the cents whole over its dot-led .fraction, or a power over its "(max)". With NO sub the main is
    a solo glyph at the full cell font (the rtt-stacked-solo class); with one it is the reduced
    whole-part size over the small sub."""
    solo = "" if sub else " rtt-stacked-solo"
    sub_html = f'<div class="rtt-stacked-sub">{_escape(sub)}</div>' if sub else ""
    return (f'<div class="rtt-tuning-value">'
            f'<div class="rtt-stacked-main{solo}">{_escape(main)}</div>{sub_html}</div>')


def _ratio_face_html(text: str, width: float, approx: bool, cell_font: float) -> str:
    """The read-only stacked-fraction face as HTML, mirroring app._ratio_body: an optional ~ before a
    .rtt-frac num/bar/den, a whole "n/1" collapsed to a bare big integer (.rtt-frac-whole), and a
    non-number (dash / blank) shown bare. The digits' inline font matches _fit_ratio (the whole
    collapse caps at the value-cell font, a true fraction at _RATIO_MAX_FONT) so the magnifier scales
    the very face the cell shows."""
    parts = _ratio_parts(text)
    if parts and not all(p.lstrip("-").isdigit() for p in parts):
        parts = None  # a dashed "—/—" isn't a number — render it bare
    if not parts:
        return f'<div class="rtt-ratio"><div class="rtt-value">{_escape(text)}</div></div>'
    num, den = parts
    whole = den == "1"  # "n/1" -> a bare big integer (collapse like the cell)
    size = _digit_fit_font(len(num), width, cell_font) if whole else _ratio_font(num, den, width)
    tilde = '<div class="rtt-approx">~</div>' if approx else ""
    frac_cls = "rtt-frac rtt-frac-whole" if whole else "rtt-frac"
    return (f'<div class="rtt-ratio">{tilde}'
            f'<div class="{frac_cls}">'
            f'<div class="rtt-frac-num" style="font-size:{size:.2f}px">{_escape(num)}</div>'
            f'<div class="rtt-frac-den" style="font-size:{size:.2f}px">{_escape(den)}</div>'
            f'</div></div>')


def _zoom_face_html(kind: str, text: str, width: float, cell_font: float) -> str:
    """One read-only gridded value's face as HTML, by kind (see :data:`ZOOM_KINDS`) — the same face
    the cell renders, so the magnifier shows a faithful enlarged copy."""
    if kind == "tuningvalue":  # the stacked cents whole over its .fraction
        whole, frac = _cents_parts(text)
        return _stacked_face_html(whole, f".{frac}" if frac else "")
    if kind == "powerdisplay":  # a stacked power: ∞ over "(max)", or a bare number
        return _stacked_face_html(*_power_parts(text))
    if kind in ("genratio", "commaratio"):  # a stacked fraction (genratio shows the ~ approximate)
        return _ratio_face_html(text, width, approx=(kind == "genratio"), cell_font=cell_font)
    if kind == "mathexpr":  # a value's closed form, already a width-fitted stacked HTML
        return _mathexpr_html(text, width)
    return _escape(text)  # defensive: an unlisted kind shows its bare text


def zoom_tip_html(kind: str, text: str, width: float, cell_font: float) -> str:
    """The full magnifier-tooltip body: the value's face (:func:`_zoom_face_html`) wrapped in the
    fixed .rtt-zoom-box square that the .rtt-zoom-scale transform enlarges. Every value face fits a
    COL_W cell, so the scaled copy fits the scaled box."""
    return (f'<div class="rtt-zoom-box"><div class="rtt-zoom-scale">'
            f'{_zoom_face_html(kind, text, width, cell_font)}</div></div>')
