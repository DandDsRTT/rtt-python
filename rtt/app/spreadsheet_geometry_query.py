...
from rtt.app.spreadsheet_geometry_bands import (
    canonical_top,
    comma_picker_band_y,
    frame_foot_y,
    frame_top_y,
    ...
    subrow_top,
    superspace_map_top,
    superspace_projection_top,
    superspace_vector_top,
    vector_top,
)
from rtt.app.spreadsheet_text import SCHEME_LABEL_WIDTH, _sub, _subscript_coord, _wrap_lines
...
def basis_col_x(geometry) -> float:
    return (
        geometry.column_x["quantities"] + (geometry.column_width["quantities"] - COLUMN_WIDTH) / 2
    )


_INTERVAL_X_ATTR = {"targets": "targets_x", "interest": "interest_x", "held": "held_x"}
_PROJECTION_MATRIX_ATTR = {"primes": "matrix", "generator_embedding": "embedding_matrix"}
...
def plain_text_editable(resolved, row_key: str, column_key: str) -> bool:
    ...
    if row_key == "vectors" and column_key == "generator_embedding":
        return projection_cells_editable(resolved, column_key)
