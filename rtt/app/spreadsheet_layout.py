...
def _col_bands(geometry, resolved, context):
    return (
        ...
        ("generators", 2 * BRACKET_WIDTH + resolved.dimensions.generator_count_shown * COLUMN_WIDTH + 2 * query.matrix_label_gutter_width(geometry, "generators"), resolved.flags.temperament_tiles),
...
def _group_geometry_fields(geometry, resolved):
    group_n = {"generators": resolved.dimensions.generator_count_shown, "primes": resolved.dimensions.dimensionality_shown, "commas": resolved.dimensions.vector_count_shown,
...
def _init_group_geometry(geometry, resolved, context) -> Geometry:
    ...
    plus_stub_x = {column_key: query.column_plus_x(geometry, resolved, column_key)
                   ...
                   if query.plus_shows(geometry, resolved, context.collapsed, column_key)}
    row_plus_y = {}
