...
# and the green _PENDING_COLOR add-preview, so the three state hues read apart at a glance
_ALERT_COLOR = "#e53935"  # red for an alerting cell (.rtt-alert: a held interval the current
# tuning no longer holds just). Was once shared with the pending draft via _PENDING_COLOR; split
# off when drafts went green, so "now invalid" stays red while "being created" reads green
...
class _Reconciler:
    ...
    def _update_commacell(self, cb):
        if cb.pending:  # the draft column: show the typed component (blank if None), green-outlined
