...
def add_mapping_row_to(state: TemperamentState, source: int, target: int) -> TemperamentState:
    ...
    mapping row becomes ``row[target] + row[source]``. A row operation that holds the temperament
    (its comma basis, rank and nullity) — only the generator basis changes, re-expressing the same
    tuning over a different generating set. The dragged generator's ratio shifts (e.g. the octave
    dropped onto the fifth becomes a fourth) while the target's stays put; callers transform any
    frozen generator tuning to hold the sound. Dropping a row onto itself (``source == target``)
    doubles it — the one non-unimodular case, deliberately enfactoring the mapping into a contorted
    temperament that still holds the same tuning (callers halve that frozen generator)."""
...
def add_comma_to(state: TemperamentState, source: int, target: int) -> TemperamentState:
    ...
    column operation on the comma basis that holds the temperament (its mapping, rank and nullity);
    only which intervals name the nullspace changes. The mapping is the comma basis's dual and is
    unaffected, so unlike the row op no tuning transform is needed. Dropping a comma onto itself
    (``source == target``) doubles its vector — an enfactored basis that still spans the same
    nullspace, so the mapping is unchanged."""
