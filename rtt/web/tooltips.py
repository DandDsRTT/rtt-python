...
_KIND_HELP: dict[str, str] = {
    ...
    "map_drag": "Drag this generator (a mapping row) onto another to add it into that row — or onto itself to double it — a change of generator basis that holds the temperament and its tuning.",
    "int_drag": "Drag this interval onto another in the same column — or onto itself to double it — to combine them into their product. For the comma basis this re-expresses the same temperament; for a target / held / interest list it just combines the two intervals.",
