...
def test_projection_on_adds_a_dxd_matrix_box_in_the_tuning_section():
    cells = {c.id: c for c in _with(projection=True).cells}
    assert cells["label:projection"].text == "projection"
    # a d x d matrix of read-only cells (d=3 here), on the shared domain-prime axes
    for i in range(3):
        for p in range(3):
            assert cells[f"cell:proj:{i}:{p}"].kind == "static"
            assert cells[f"cell:proj:{i}:{p}"].x == cells[f"cell:mapping:0:{p}"].x  # prime columns
    # its own row, placed between the mapping and the tuning rows (per the mockup)
...
def test_projection_box_is_a_stub_identity_for_now():
    # the projection computation is deferred; the box shows the d x d identity as a
    # read-only placeholder until the real P = GM projection is wired up
    cells = {c.id: c for c in _with(projection=True).cells}
    for i in range(3):
        for p in range(3):
            assert cells[f"cell:proj:{i}:{p}"].text == ("1" if i == p else "0")
...
def test_projection_row_has_a_horizontal_gridline_like_the_other_rows():
    lay = _with(projection=True)
    by_id = {ln.id: ln for ln in lay.lines}
    cells = {c.id: c for c in lay.cells}
    assert "h:projection" in by_id  # the row gets a gridline so the ruled grid stays unbroken
    line, label = by_id["h:projection"], cells["label:projection"]
    assert label.y <= line.pos <= label.y + label.h  # centred on the projection row band
