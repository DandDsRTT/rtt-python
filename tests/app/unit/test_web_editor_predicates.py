...
def test_structure_predicates_are_pure_functions_of_state():
...
def test_valid_domain_basis_accepts_a_standard_prime_limit():
    assert ep.valid_domain_basis(service.from_mapping([[1, 1, 0], [0, 1, 4]])) is True


def test_list_vectors_reads_each_named_collection_off_the_read_model():
    editor = Editor()
...
def test_peek_vector_returns_none_out_of_bounds():
    assert ep.peek_vector([(1, 0)], 0) == (1, 0)
    assert ep.peek_vector([(1, 0)], 5) is None
    assert ep.peek_vector([], 0) is None


def test_move_feasible_blocks_into_unchanged_and_allows_a_plain_move():
    s = solve_model(Editor())
