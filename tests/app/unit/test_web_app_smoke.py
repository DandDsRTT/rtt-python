...
class TestWebAppSmoke1:
    ...
    def test_columnfill_twins_track_the_scroll_on_the_compositor_like_the_header_so_a_fling_shows_no_ghost(self):
        ...
        assert "@keyframes rtt-fill-track" in css and "translateX(calc(-100% + 100cqw - var(--pad)\n" in css, "the twins ride the horizontal scroll on a compositor scroll-PROGRESS timeline (mirroring the header's rtt-head-track), so a fast fling can't lag them out from behind their live rules as a ghosted second set of verticals. The end translate is the header's minus one _PAD, since this clip runs a _PAD wider (right:0 vs the header's right:pad)"
        assert css.count("- var(--rtt-scroll-slack, 0px)") == 2, "both keyframes subtract the slack freeze.js publishes — a space-taking scrollbar and the right padding fit() drops are the only two lengths the pure-layout calc cannot see from outside the scroller, and either one leaves the layers short of the body's real max scrollLeft (see tests/app/integration/test_browser_gridline_echo.py)"
...
class TestWebAppSmoke3:
    ...
    def test_every_editable_kind_is_a_value_kind(self):
        assert page_assets.GRIDVALUE_KINDS <= spreadsheet_constants.VALUE_KINDS, "the editable set is a subset of all value kinds; a gridvalue that is not a value kind would skip the value-kind wiring"
