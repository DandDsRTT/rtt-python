...
def test_building_guard_sets_true_inside_and_restores_false_outside():
    rt = PageRuntime()
...
def test_building_guard_is_reentrant_and_restores_the_prior_true_value():
    rt = PageRuntime()
    with rt.building_guard():
        with rt.building_guard():
            assert rt.building is True
...
def test_building_guard_restores_even_when_the_body_raises():
    rt = PageRuntime()
    try:
        with rt.building_guard():
...
def test_set_chapter_clamps_into_range_and_defaults_garbage():
    rt = PageRuntime()
...
def test_col_tokens_reads_the_identities_of_the_named_axis():
    rt = PageRuntime()
...
def test_token_index_locates_a_cells_token_within_its_axis():
    rt = PageRuntime()
...
def test_available_keys_filters_implemented_by_current_chapter():
    rt = PageRuntime()
...
def test_dark_icon_reflects_dark_mode():
    rt = PageRuntime()
...
def test_bind_client_records_the_captured_page_client():
    rt = PageRuntime()
