...
class TestBracketSingleSource:
    def test_no_open_value_tile_draws_two_overlapping_outer_bracket_structures(self):
        from rtt.app.grid_tables import PLAIN_TEXT_ROWS, SPINE_COLUMNS
        b = _maximized_superspace_builder()
        layout = b.layout()
        value_rows = PLAIN_TEXT_ROWS - {"quantities"}
        ...
        for (row_key, column_key) in sorted(b.geometry.declared_tiles):
            if row_key not in value_rows or column_key in SPINE_COLUMNS or not query.tile_open(b.geometry, b.inputs.collapsed, row_key, column_key):
                continue
            frame_top, _col_marks, _curve, _angle, outer, _perrow = _ebk_grid_structures(b, layout, row_key, column_key)
            if frame_top and outer:
                offenders.append((row_key, column_key, "matrix_frame + spanning outer bracket overlap"))
            elif len(outer) > 1:
                offenders.append((row_key, column_key, f"{len(outer)} spanning outer brackets"))
            checked += 1
        assert checked >= 60, f"config did not light enough value tiles ({checked})"
        assert not offenders, "tiles drawing redundant overlapping bracket structures:\n" + "\n".join(
            f"  {r}/{c}: {why}" for r, c, why in offenders)
