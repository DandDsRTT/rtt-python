...
def test_gesture_controller_constructs_without_a_page():
...
def test_compute_rings_empty_when_preview_highlighting_off():
...
def test_paint_cell_is_a_noop_when_signature_unchanged():
    el = _FakeEl()
    ent = _FakeEntity(el)
...
def test_end_gesture_restores_preview_token():
    ed = _RecordingEditor()
...
def test_end_commit_gestures_ends_transient_gesture_and_clears_rank():
    ed = _RecordingEditor()
    g = GestureController(ed, SimpleNamespace())
...
def test_end_commit_gestures_leaves_an_edit_gesture_alone():
