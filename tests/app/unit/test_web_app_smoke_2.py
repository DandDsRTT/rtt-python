...
class TestWebAppSmoke3:
    ...
    def test_interest_example_is_the_bold_interval_symbol(self):
        assert render_html._math_html("𝐢") in render_html._example_html("interest"), (
            "the mockup labels each interval-of-interest 𝐢 (bold upright, like the vectors), so # the toggle's example shows that same glyph"
        )
    ...
    def test_general_tile_covers_every_general_layer_exactly_once(self):
        ...
        covered = (
            [key for line in page_assets._GENERAL_TILE_LINES for key in line]
            + list(page_assets._TILE_IN_CELL_LAYERS)
            + list(page_assets._TILE_HEAD_LAYERS)
        )
    ...
    def test_general_tile_rides_each_subcontrol_on_its_parents_line(self):
        ...
        general_subs = {
            k: p
            for k, p in show_settings.SUBCONTROLS.items()
    ...
    def test_general_tile_seats_the_value_layers_inside_the_gridded_cell(self):
        value_line = next(
            line for line in page_assets._GENERAL_TILE_LINES if "gridded_values" in line
        )
        assert set(value_line) == {
            "gridded_values",
            "math_expressions",
            "quantities",
            "decimals",
    ...
    def test_general_tile_symbol_and_equivalence_read_as_one_equation(self):
        assert render_html._general_part_html("symbols") == render_html._math_html(
            render_html._TILE_SYMBOL
        ), (
            "the symbol part is the bold-italic n; the equivalence part is its defining-equation tail, # so the two joined read 𝒏 = 𝑒G (the symbol's equation), one source of truth with _TILE_*"
        )
        assert render_html._general_part_html("symbols") + render_html._general_part_html(
            "equivalences"
    ...
    def test_row_titles_stack_one_word_per_line_and_select_as_one_unit(self):
        ...
        assert "width:min-content" in rule, (
            "min-content wraps the title at every space — one word per line — while the copied text # stays a single spaced line (a soft wrap carries no newline, an embedded \\n would)"
        )
        assert "user-select:all" in rule, (
            "any click — including a double-click — selects the whole title, so a copy grabs # 'superspace interval vectors' rather than one word or one line of it"
        )
    ...
    def test_every_option_square_renders_at_one_uniform_size(self):
        ...
        assert ".rtt-control-check .q-checkbox__inner" not in page_assets._CSS, (
            "the per-control overrides that made the in-grid control checkboxes oversized are gone — # the universal rules above now size every checkbox, so nothing re-diverges"
        )
    ...
    def test_option_checkbox_renders_as_one_svg_for_zoom_stable_appearance(self):
        assert page_assets._CSS.count("data:image/svg") == 6, (
            "The checkbox + fill is ONE SVG background that scales as a coherent vector — square with an even # border at any zoom — instead of a CSS border + inset ::after fill, whose edges snap to the # device-pixel grid independently and distort the square / gap at fractional zooms (the reported # zoom-dependent jank). The checked SVG carries the black fill, the mixed master a grey fill, the # unchecked checkbox only the outline; the tuning-ranges radio checkbox reuses the same art. # the checkbox art is one SVG per state, defined once as a :root custom property and referenced # everywhere (so the same vector backs the checkbox, the mixed master, and the range checkbox)"
        )
        bg = _css_rule(".q-checkbox__bg")
        assert "var(--option-checkbox-unchecked)" in bg and "border:none" in bg
        assert "var(--option-checkbox-checked)" in _css_rule(
            '.q-checkbox[aria-checked="true"] .q-checkbox__bg'
        )
        assert "var(--option-checkbox-disabled)" in _css_rule(".rtt-show-mixed .q-checkbox__bg")
        assert "var(--option-checkbox-unchecked)" in _css_rule(".rtt-rangebar")
    ...
    def test_bar_chart_indicator_line_is_broken_by_its_power_labelled_mean_damage(self):
        svg = render_html._bar_chart(
            272, 64, (0.0, 10.0, 26.385), indicator=26.385, indicator_label="∞"
        )
        assert svg.count(f'stroke="{render_html._CHART_INDICATOR}"') == 2, (
            "the line is drawn in two segments (a stub left of the label, the rest to its right), # leaving the gap the label fills — not one unbroken rule"
        )
    ...
    def test_bar_chart_renders_numerically_flat_dust_without_dividing_by_zero(self):
        ...
        bars = _bars(svg)
        assert len(bars) == 3
        assert all(abs(height) < 0.01 for _y, height in bars), (
            "dust rests on the baseline, not blown up"
        )
    ...
    def test_range_chart_ticks_the_live_tuning_within_a_generators_range(self):
        marks = sorted(
            y
            for y, height in _bars(render_html._range_chart(92, 96, ((685.714, 720.0),), (697.0,)))
            if height < 4
        )
        assert len(marks) == 3
        ...
        plain = sorted(
            y
            for y, height in _bars(render_html._range_chart(92, 96, ((685.714, 720.0),)))
            if height < 4
        )
        assert len(plain) == 2
...
class TestChecklistExamples:
    ...
    def test_gridded_cell_and_brackets_examples_share_one_scale(self):
        def scale(key):
            return re.search(r"transform:scale\(([\d.]+)\)", render_html._example_html(key)).group(
                1
            )
...
class TestWebAppSmoke4:
    ...
    def test_plain_text_font_shrinks_to_fit_with_no_readability_floor(self):
        ...
        assert (
            render_html._plain_text_font("1 0 0", 120) == spreadsheet_constants.PLAIN_TEXT_MAX_FONT
        )
        assert (
            render_html._plain_text_font("x" * 9, 30) <= spreadsheet_constants.PLAIN_TEXT_MAX_FONT
        )
    ...
    def test_dense_prescaling_plain_text_fits_its_cell(self):
        ...
        cells = {
            ...
            for c in spreadsheet.build(
                service.from_mapping(((1, 1, 0), (0, 1, 4))), s, tuning_scheme="TILT minimax-S"
            ).cells
        }
        for cell_id in ("plain_text:prescaling:primes", "plain_text:prescaling:targets"):
            c = cells[cell_id]
            assert (
                render_html._plain_text_units(c.text)
                * render_html._plain_text_font(c.text, c.width)
                <= c.width
            ), cell_id
    ...
    def test_units_fit_their_cell_for_long_alternative_complexity_annotations(self):
        ...
        cells = spreadsheet.build(
            service.from_mapping(((1, 1, 0), (0, 1, 4))), s, tuning_scheme="TILT minimax-E-sopfr-S"
        ).cells
        shrunk = False
        for c in cells:
            if c.kind == "units":
                font = render_html._units_font(c.text, c.width, page_assets._UNITS_MAX_FONT)
                assert len(c.text) * render_html._EXPR_CHAR_W * font <= c.width, (
                    f"units {c.id}={c.text!r}"
                )
            ...
            if c.unit:
                ...
                assert len(c.unit) * render_html._EXPR_CHAR_W * font <= c.width, (
                    f"cellunit {c.id}={c.unit!r}"
                )
...
class TestGuidedTour:
    def test_tour_steps_are_well_formed_and_assets_wired(self):
        ...
        for step in page_assets._TOUR_STEPS:
            ...
            assert selection == "" or selection[0] in ".[", (
                f"selector should be a real CSS selector # (class or attribute), not a test-only .mark(), got {selection!r}"
            )
    ...
    def test_first_run_opens_at_the_minimum_chapter(self):
        assert app._initial_chapter({}) == show_settings.CHAPTER_MIN, (
            "a genuinely-new browser opens at the minimum chapter (the reset state), not chapter 4"
        )
        assert (
            app._initial_chapter({page_assets._STORE_KEY: "saved"}) == show_settings.CHAPTER_MIN
        ), "a saved document with no explicit chapter still opens at the minimum chapter"
    ...
    def test_mapping_step_frames_the_whole_mapping_in_plain_words(self):
        ...
        assert step.get("region") is True, (
            "the spotlight frames the WHOLE mapping matrix, not one cell"
        )
        assert ids.mapping_cell("0", 0).startswith("cell:mapping:"), (
            "the selector must match a real # mapping cell id, not a test-only .mark()"
        )
    ...
    def test_tour_arms_mapping_demos_itself_so_the_grid_lesson_stays_hop_free(self):
        ...
        assert "Switch on mapping demos" not in titles, (
            "the tour arms mapping demos itself (tour_begin) # so the early grid lesson never detours into the settings panel to flip it on"
        )
        assert "mapping demos" in _tour_step("App features")["body"].lower(), (
            "the panel step still names # mapping demos so the learner sees where the switch that drew the animations lives"
        )
    ...
    def test_tempering_out_step_frames_the_whole_comma_for_the_hover_demo(self):
        ...
        assert step.get("interact") is True, (
            "the concept is taught by doing: tour.js drops the scrim's # pointer-events so the learner's hover reaches the comma and the mapping demo animates"
        )
        assert step["selector"].startswith(".rtt-cell") and "cell:comma:" in step["selector"]
        assert step.get("region") is True, (
            "the spotlight frames the WHOLE comma basis, not one cell"
        )
        assert step.get("gate") == "demo", (
            "Next stays blocked until the learner actually hovers and the # demo draws — teaching by doing, not clicking past"
        )
        assert ids.comma_cell("0", 0).startswith("cell:comma:"), (
            "the selector must match a real comma # cell id, not a test-only .mark()"
        )
        assert "temper" in step["body"].lower() and "vanish" in step["body"].lower()
        assert "[0 0]" in step["body"], (
            "the payoff is the comma mapping to the all-zeros generator-count # vector the guide writes [0 0]"
        )

    def test_try_an_edit_step_is_restored_and_hands_on(self):
        step = _tour_step("Try an edit")
        assert step.get("interact") is True and "cell:mapping:" in step["selector"], (
            "the learner # edits a real mapping cell and watches the grid recompute — restored from the old tour"
        )
    ...
    def test_learner_raises_the_chapter_themselves(self):
        ...
        assert step.get("interact") is True, (
            "the learner drives the real chapter slider up themselves — # an interact step so the drag reaches the control"
        )
        assert step.get("gate") == "chapter4", (
            "Next stays blocked until they actually reach chapter 4"
        )
    ...
    def test_gated_steps_block_next_until_the_learner_acts(self):
        ...
        assert "armGate" in js and "next.disabled" in js, (
            "an interact step disables Next until its gate # is met, so the tour blocks progress until the learner does the thing"
        )
        gated = {
            step["title"]: step["gate"] for step in page_assets._TOUR_STEPS if step.get("gate")
        }
        assert gated == {
            "Tempering out": "demo",
            "Try an edit": "edited",
            "Reveal more, chapter by chapter": "chapter4",
        }, gated
        for title in gated:
    ...
    def test_reshaping_and_undo_and_panel_steps_survive_the_rewrite(self):
        for title in ("Reshaping the grid", "Undo, reset & share", "Tile features", "App features"):
            ...
            assert step["title"] and step["body"], (
                f"good onboarding step {title!r} must stay in the tour"
            )
    ...
    def test_tour_visits_the_grid_before_the_panel_with_a_single_pane_switch(self):
        ...
        assert opens == list(range(opens[0], len(page_assets._TOUR_STEPS) - 1)), (
            "every drawer step is # contiguous at the end — the grid lesson finishes first, then the panel, one switch, no hopping"
        )
    ...
    def test_tour_bridges_begin_skip_and_complete(self):
        js = page_assets._TOUR_JS
        assert 'emit("rtt_tour_begin")' in js, (
            "start() snapshots the learner's work and resets to ch2"
        )
        assert '"rtt_tour_skip"' in js and '"rtt_tour_complete"' in js, (
            "both exits restore the sandbox — # skip lands at ch2, complete at the full app"
        )
    ...
    def test_skip_lands_at_ch2_and_complete_at_the_full_app(self):
        ...
        assert 'abort === false ? "rtt_tour_complete" : "rtt_tour_skip"' in js, (
            "an abort (skip/Escape) # returns to ch2; the end completes to the full app — both restore the learner's own work"
        )
    ...
    def test_tour_owns_the_arrow_keys_gated_and_the_grid_yields_them(self):
        ...
        assert "gateSatisfied(step.gate)" in tour_js and "ArrowRight" in tour_js, (
            "ArrowRight advances # only when the step's gate is satisfied, exactly like the Next button"
        )
        assert "rtt-tour-running" in active_js, (
            "the grid's active-cell arrow-roam yields to the tour so # a single arrow press never both advances the tour AND moves the grid cursor"
        )

    def test_tour_region_step_frames_every_matched_cell_not_just_the_first(self):
        js = page_assets._TOUR_JS
        assert "step.region" in js and "querySelectorAll" in js, (
            "a region step unions every matched # cell into one spotlight so the whole matrix is framed"
        )
        ...
        for step in region_steps:
            assert step["selector"].startswith(".rtt-cell["), (
                "a region step targets a family of cells"
            )

    def test_tour_silences_the_apps_incidental_hover_affordances_while_running(self):
        js = page_assets._TOUR_JS
        assert 'classList.add("rtt-tour-running")' in js, (
            "start() flags the body while the tour runs"
        )
        ...
        for surface in (".rtt-zoom-overlay", ".q-tooltip", ".rtt-zoom-help", ".rtt-guide-card"):
            assert f"body.rtt-tour-running {surface}" in css, (
                f"the tour hides {surface} so the # tempering hover shows only the tour's own card, spotlight and mapping-demo overlay"
            )
    ...
    def test_a_brand_new_browser_opens_at_the_minimum_chapter(self):
        ...
        assert app._initial_chapter({}) == show_settings.CHAPTER_MIN, (
            "first-run for a genuinely-new # browser opens at the simplest chapter-2 start, matching Reset — not chapter 4"
        )
    ...
    def test_tour_begin_teaches_from_a_clean_chapter_two_default_with_the_demo_armed(
        self, monkeypatch
    ):
        ...
        assert page.editor.settings["mapping_demos"] is True, (
            "the demo is armed so the hover step works # without a settings detour"
        )
        assert page.editor.settings["tile_units"] is False, (
            "the tour teaches on a clean default (the 81/80 # lesson and the edit step land on a throwaway), so no step points at a feature already on"
        )
        assert (
            page.editor.settings["tuning"] is False and page.editor.settings["interest"] is False
        ), "chapter 2 is the simplest grid"
    ...
    def test_tour_is_a_sandbox_restoring_the_learners_work_when_they_leave(self, monkeypatch):
        ...
        app._Page.tour_begin(page)
        assert page.editor.settings["optimization"] is False, (
            "the tour teaches on the clean default"
        )
        app._Page.tour_exit(page, show_settings.CHAPTER_DEFAULT)
        assert page.editor.settings["optimization"] is True, (
            "leaving the tour restores the learner's own # work — the reset was only a sandbox for the lesson, never a real edit to their document"
        )
        assert page.editor.settings["mapping_demos"] is False, (
            "and the tour's armed demo is put back the # way the learner had it"
        )
        assert page.runtime.chapter == show_settings.CHAPTER_DEFAULT, (
            "completing lands at the full app; # skip (tour_exit with CHAPTER_MIN) would land at the simple chapter-2 start instead"
        )
    ...
    def test_tour_begin_persists_the_chapter_two_start_like_a_reset(self, monkeypatch):
        ...
        page = _tour_page(monkeypatch, store)
        app._Page.tour_begin(page)
        assert store[page_assets._CHAPTER_KEY] == show_settings.CHAPTER_MIN, (
            "chapter 2 is now a real # resting state (Reset lands there too), so the tour's clean start persists it like any chapter"
        )

    def test_raising_the_chapter_re_reveals_the_default_layers_lowering_hides_them(
        self, monkeypatch
    ):
        page = _tour_page(monkeypatch, {})
        app._Page.on_chapter_change(page, show_settings.CHAPTER_MIN)
        assert page.editor.settings["tuning"] is False, (
            "lowering to chapter 2 hides the advanced layers"
        )
        app._Page.on_chapter_change(page, show_settings.CHAPTER_DEFAULT)
        assert page.editor.settings["tuning"] is True, (
            "the chapter slider is non-lossy: raising it re-reveals # the default layers, so the grid fills back in as you ramp up from the chapter-2 beginning"
        )
    ...
    def test_tour_autostart_is_desktop_first_but_replay_is_always_available(self):
        ...
        assert "!seen()" in autostart and "wideEnough()" in autostart, (
            "autostart is gated on width (alongside # any other autostart guards)"
        )
        assert "window.rttTour.start = start" in js, (
            "the ? replay button (start) stays available at any width"
        )
    ...
    def test_reset_returns_to_chapter_two_and_clears_the_tour_seen_flag(self, monkeypatch):
        ...
        assert page.runtime.chapter == show_settings.CHAPTER_MIN, (
            "Reset returns to the simple chapter-2 # beginning"
        )
        assert any("rttTour" in js and "forget" in js for js in calls), (
            "Reset also clears rttTourSeen # (window.rttTour.forget) so it genuinely restores the first-run onboarding, not just the grid"
        )
