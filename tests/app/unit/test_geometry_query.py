...
def _row(y, height=10.0, frame=2.0, symbol=3.0, text=4.0, units=5.0, comma_picker=0.0):
    return RowBand(y=y, height=height, label="", tile_height=0.0, tile_top=0.0,
                   frame=frame, symbol=symbol, text=text, units=units, plain_text=0.0, preset=0.0,
...
class TestGeometryQuery:
    ...
    def test_column_identity_queries_are_pure_over_resolved(self):
        r = SimpleNamespace(
            dimensions=SimpleNamespace(comma_count=2, target_count=3, held_count=0, interest_count=0, dimensionality=5, rank=2, canonical_rank=2),
