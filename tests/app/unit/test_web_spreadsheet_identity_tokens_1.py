...
class TestColumnTokens:
    ...
    def test_pending_token_never_collides_with_a_live_column(self):
        assert spreadsheet_text.pending_token([]) == 0, (
            "the draft column's token is one past every committed column's, so it can't clash with a # surviving column even after a removal leaves a gap in the token sequence; an empty list's # draft is 0 (so a first pending vector cell is …:0, as the index-keyed tests expect)"
        )
    ...
    def test_basis_groups_claim_freed_slots_positionally_on_a_resolve(self):
        ...
        dropped = spreadsheet_text.assign_column_tokens(
            previous, [(12, 19, 28)], claim_unmatched=True
        )
    ...
    def test_reordered_held_column_keeps_its_vector_cell_id_and_glides(self):
        ...
        assert c2["cell:held:0:2"].text == c1["cell:held:0:2"].text, (
            "carrying its own content, not slot 0's"
        )
    ...
    def test_reordering_held_rekeys_every_column_cell_not_just_the_vectors(self):
        ...
        moved = {
            cell_id
            for cell_id in spreadsheet_text.changed_cell_ids(lay1, lay2)
            if not _reorder_volatile(cell_id)
        }
    ...
    def test_reordering_interest_rekeys_its_column_cells(self):
        ...
        lay2 = spreadsheet.build(
            _held_state(),
            _all_on(),
            interest=[interest[2], interest[0], interest[1]],
            previous_ids=lay1.identities,
        )
        moved = {
            cell_id
        ...
        assert moved == set(), (
            f"interest cells re-filled in place instead of gliding: {sorted(moved)}"
        )
    ...
    def test_reordering_targets_rekeys_its_column_cells(self):
        ...
        lay2 = spreadsheet.build(
            _held_state(),
            _all_on(),
            target_override=(targets[2], targets[0], targets[1]),
            previous_ids=lay1.identities,
        )
        moved = {
            cell_id
        ...
        assert moved == set(), (
            f"target cells re-filled in place instead of gliding: {sorted(moved)}"
        )
    ...
    def test_removing_a_column_keeps_the_survivors_identity_so_they_do_not_ring(self):
        ...
        lay1 = spreadsheet.build(_held_state(), _all_on(), interest=interest)
        lay2 = spreadsheet.build(
            _held_state(), _all_on(), interest=interest[1:], previous_ids=lay1.identities
        )
    ...
    def test_mapping_cells_form_a_square_touching_grid(self):
        ...
        assert (
            cells["cell:mapped:0:1"].x == m00.x + m00.width + spreadsheet_constants.INTERVAL_COL_GAP
        )
    ...
    def test_mapping_rejoin_bars_span_the_full_generator_fan(self):
        ...
        assert (
            right.start == g0.position - half
            and right.start + right.length == glast.position + half
        )
    ...
    def test_adjacent_tiles_keep_a_roomy_minimum_gap(self):
        ...
        assert (
            bot.y - (top.y + top.height)
            == spreadsheet_constants.GAP - 2 * spreadsheet_constants.PAD
        )
    ...
    def test_axis_ids_are_stable_across_expand(self):
        before = {
            line.id
            for line in spreadsheet.build(service.from_mapping(((1, 1, 0), (0, 1, 4)))).lines
        }
...
class TestSpineAndAxes:
    ...
    def test_generators_column_fans_into_per_generator_axes(self):
        ...
        assert by_id["trunk:generators"].length < by_id["trunk:quantities"].length, (
    ...
    def test_gridded_values_off_empties_the_value_cells_but_keeps_brackets_controls_and_structure(
        self,
    ):
        ...
        assert {
            ...
            "target_minus:0",
            "target_plus",
        } <= ids, "the ± controls carry no value, so gridded-values off leaves them"
        assert {
            "label:mapping",
            "header:primes",
            "header:targets",
    ...
    def test_add_buttons_survive_every_gridded_values_and_ebk_combination(self):
        ...
        for gridded in (True, False):
            for ebk in (True, False):
                ...
                assert buttons <= ids, (
                    f"add buttons vanished at gridded_values={gridded}, ebk={ebk}"
                )
    ...
    def test_brackets_off_removes_the_enclosure_everywhere_keeping_values_and_controls(self):
        ...
        assert any(cid.startswith("cell:mapping:") for cid in off), (
            "the values stay, just unenclosed"
        )
        assert {"plus", "minus", "comma_plus"} <= off.keys(), (
            "the ± controls carry no bracket, so they stay"
        )
    ...
    def test_brackets_presence_is_independent_of_the_ebk_notation_style(self):
        ...
        for ebk in (True, False):
            ...
            assert shown & enclosure_kinds, (
                "with brackets on, the enclosure shows in either notation style"
            )
    ...
    def test_general_quantities_off_blanks_the_body_numbers_keeping_boxes_and_brackets(self):
        ...
        for cell_id in ("prime:0", "comma:0", "target:0"):
            assert (
                cell_id in off
                and not on[cell_id].blank
                and off[cell_id].blank
                and off[cell_id].text == ""
            )
    ...
    def test_temperament_tiles_off_removes_the_mapping_row_and_domain_columns(self):
        ...
        assert "label:vectors" in off, (
            "the interval-vectors row owns its own toggle now (interval_vectors), so it does NOT go with # the temperament tiles: it stays, still showing the target vectors over the surviving targets # column. Only the cells in the now-gone temperament columns (the comma vectors) vanish"
        )
        assert "cell:vector:targets:0:0" in off
        assert "header:primes" not in off
        assert not any(
            c.startswith(("prime:", "tuning:prime:", "just:prime:", "retune:prime:")) for c in off
        )
        assert "header:commas" not in off, (
            "the commas column belongs to the temperament too, so it goes as well: header, # comma headers, the comma basis (its vectors), and the comma-size cells across the tuning rows"
        )
        assert not any(
            c.startswith(("comma:", "cell:comma:", "tuning:comma:", "just:comma:", "retune:comma:"))
            for c in off
        )
    ...
    def test_interval_vectors_off_removes_the_interval_vectors_row_only(self):
        ...
        assert not any(
            c.startswith(("cell:vector:", "cell:comma:", "cell:interest:", "cell:held:"))
            for c in off
        )
        assert {
            "label:mapping",
    ...
    def test_interval_vectors_row_reserves_no_phantom_picker_band_when_commas_column_hidden(self):
        ...
        assert (
            with_comma["block:vector:quantities"].height
            == no_comma["block:vector:quantities"].height
        ), (
            "with the commas column hidden, the comma in state must not grow the vectors row: its spine # tile is the same height as the no-comma build (which reserves no band either way)"
        )
...
class TestFormBox:
    ...
    def test_form_box_shows_the_inverse_form_matrix_over_the_generators(self):
        ...
        assert (
            ...
            and cells["cell:inverse_form:0:1"].text == "-1"
        ), (
            "the canonical row's generators tile is 𝐹⁻¹ (𝑀_C = 𝐹⁻¹𝑀, g_C/g) — read-only; the EDITABLE 𝐹 (𝑀 = 𝐹𝑀_C) # rides the mapping row's canonical-generators column instead. For ((1,1,0),(0,1,4)), 𝐹⁻¹ = ((1,-1),(0,1))"
        )
        assert (
            cells["cell:inverse_form:1:0"].text == "0"
    ...
    def test_canonical_generators_column_sits_between_units_and_generators(self):
        ...
        assert (
            cells["header:quantities"].x
            < cells["header:canonical_generators"].x
            < cells["header:generators"].x
        )
        with_units = {c.id: c for c in _with(form_tiles=True, app_units=True, tile_units=True).cells}
        assert (
            with_units["header:units"].x
            < with_units["header:canonical_generators"].x
            < with_units["header:generators"].x
        )
