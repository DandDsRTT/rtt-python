...
async def test_tuning_chooser_shows_the_prompt_as_a_placeholder_when_off_list(user: User) -> None:
    ...
    user.find(kind=ui.checkbox, content="optimization").click()
    user.find(kind=ui.checkbox, content="weighting").click()
    user.find(kind=ui.checkbox, content="alternative complexity").click()  # 𝑝 editable (else read-only)
...
async def test_alt_complexity_enables_and_widens_the_complexity_chooser(user: User) -> None:
    ...
    user.find(kind=ui.checkbox, content="alternative complexity").click()      # open the full measure list
    await user.should_see(marker="control:complexity")
...
async def test_all_interval_renders_the_locked_power_as_a_read_only_value(user: User) -> None:
    ...
    user.find(kind=ui.checkbox, content="alternative complexity").click()  # 𝑝 editable while target-based
...
async def test_hovering_a_non_last_comma_minus_reds_that_comma_not_the_last(user: User) -> None:
    ...
    button = set(user.find(marker="comma_minus:0").elements)
    UserInteraction(user, button, None).trigger("mouseenter")  # hover the FIRST comma's −
...
async def test_hovering_a_non_last_mapping_row_minus_reds_that_row_not_the_last(user: User) -> None:
    ...
    await user.open("/")
    button = set(user.find(marker="map_minus:0").elements)
    UserInteraction(user, button, None).trigger("mouseenter")     # hover row 0's − (meantone: 2 rows)
...
async def test_hovering_the_generator_minus_previews_the_dual_rank_change(user: User) -> None:
    ...
    await user.open("/")
    button = set(user.find(marker="gen_minus").elements)               # drop the last generator (row 1)
    UserInteraction(user, button, None).trigger("mouseenter")
...
async def test_hovering_a_column_minus_reddens_the_removed_column(user: User) -> None:
    ...
    await user.open("/")
    button = set(user.find(marker="minus").elements)                  # drop the highest prime (5)
    UserInteraction(user, button, None).trigger("mouseenter")
...
async def test_hovering_the_replace_diminuator_checkbox_previews_its_reweighting(user: User) -> None:
    ...
    user.find(kind=ui.checkbox, content="alternative complexity").click()  # ...and the size-factor checkbox
...
async def test_clicking_reset_after_hovering_it_clears_the_preview(user: User) -> None:
    ...
    button = set(user.find(marker="reset").elements)
    UserInteraction(user, button, None).trigger("mouseenter")   # hover: the hand-tuned cell would move back
...
async def test_a_control_hover_preserves_an_open_draft(user: User) -> None:
    ...
    UserInteraction(user, button, None).trigger("mouseenter")    # the hover's hypothetical runs + reverts
    UserInteraction(user, button, None).trigger("mouseleave")
...
async def test_et_picker_offers_every_uniform_map_through_72(user: User) -> None:
    # the RENDERED ET picker (not just the data) must offer every EDO's uniform maps for 1..72 — the
    # warted maps (GPVs), not only each EDO's integer uniform map — plus notable higher EDOs up to
    # 311. Guards the "every uniform map up to 72" ask.
    await _enable(user, "presets")
