...
class TestFeatureRenderBranches:
    ...
    async def test_editing_the_generator_embedding_retunes(self, user: User) -> None:
        ...
        await user.should_see(marker="plain_text:vectors:generator_embedding")
        assert _cell_child(user, "tuning:generator:1").value == "696.578"
        _cell_child(user, "plain_text:vectors:generator_embedding").set_value("{[1 0 0⟩[1/3 -1/3 1/3⟩]")
        _commit(user, "plain_text:vectors:generator_embedding")
        assert _cell_child(user, "tuning:generator:1").value == "694.786"
...
class TestProjectionPlainText:
    async def test_return_to_scheme_disables_when_the_scheme_already_matches_the_projection(self, user: User) -> None:
        await user.open("/")
        _toggle(user, "presets")
        user.find(kind=ui.checkbox, content="projection").click()
        await user.should_see(marker="scheme:primes")
    ...
    async def test_an_invalid_embedding_plain_text_toasts_and_reddens(self, user: User) -> None:
        ...
        await user.should_see(marker="plain_text:vectors:generator_embedding")
        assert _cell_child(user, "tuning:generator:1").value == "696.578"
        _cell_child(user, "plain_text:vectors:generator_embedding").set_value("{[0 0 0⟩[0 0 1/4⟩]")
        _commit(user, "plain_text:vectors:generator_embedding")
        await user.should_see("isn't a valid embedding")
        assert (
            "rtt-plain-text-error" in _cell_child(user, "plain_text:vectors:generator_embedding").classes
        )
        assert _cell_child(user, "tuning:generator:1").value == "696.578"
    ...
    async def test_the_text_form_group_starts_collapsed_and_expands(self, user: User) -> None:
        ...
        await user.should_not_see(marker="showcheckbox:names")
        user.find(marker="textformhead").click()
        await user.should_see(marker="showcheckbox:names")
        user.find(marker="textformhead").click()
        await user.should_not_see(marker="showcheckbox:names")
    ...
    async def test_the_text_form_shares_show_state_with_the_dummy_tile(self, user: User) -> None:
        ...
        user.find(marker="textformhead").click()
        user.find(marker="showpart:symbols").click()
        assert next(iter(user.find(marker="showcheckbox:symbols").elements)).value is False
        user.find(marker="showcheckbox:symbols").click()
        assert "rtt-part-on" in _part_classes(user, "symbols")
