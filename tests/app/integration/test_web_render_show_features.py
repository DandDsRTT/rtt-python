...
class TestFeatureRenderBranches:
    ...
    async def test_enabling_a_feature_renders_its_cell(self, user: User, label: str, cell_id: str) -> None:
    ...
    async def test_both_mode_shows_dd_terms_with_the_wiki_name_in_parentheses(self, user: User) -> None:
        await user.open("/")
    ...
    async def test_all_interval_scheme_dropdown_relabels_to_wiki_names_in_wiki_mode(self, user: User) -> None:
        await user.open("/")
...
class TestProjectionPlainText:
    ...
    async def test_enabling_all_interval_renders_the_target_controls_checkbox(self, user: User) -> None:
        await user.open("/")
        user.find(kind=ui.checkbox, content="optimization").click()
        user.find(kind=ui.checkbox, content="weighting").click()
        user.find(kind=ui.checkbox, content="all-interval").click()
    ...
    async def test_off_diagonal_pretransformer_edit_keeps_the_all_interval_weight_a_list(self, user: User) -> None:
        await user.open("/")
        user.find(kind=ui.checkbox, content="optimization").click()
        user.find(kind=ui.checkbox, content="weighting").click()
        user.find(kind=ui.checkbox, content="all-interval").click()
    ...
    async def test_dragging_a_target_onto_the_held_columns_gridline_moves_it_in(self, user: User) -> None:
        await user.open("/")
    ...
    async def test_hovering_a_reorder_target_previews_the_move_then_reverts(self, user: User) -> None:
        await user.open("/")
    ...
    async def test_editing_a_ratio_after_a_reorder_edits_the_column_it_heads(self, user: User) -> None:
        await user.open("/")
    ...
    async def test_sliding_the_chapter_down_disables_the_advanced_layers_in_the_grid(self, user: User) -> None:
        await user.open("/")
        slider = next(iter(user.find(marker="chapterslider").elements))
        slider.set_value(show_settings.CHAPTER_STAR)
    ...
    async def test_toggling_gridded_values_off_at_runtime_removes_the_grid_value_cells(self, user: User) -> None:
        await user.open("/")
