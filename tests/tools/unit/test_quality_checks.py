...
class TestQualityChecks1:
    def test_line_count_counts_final_unterminated_line(self):
        assert qc.line_count("a\nb\nc") == 3
        assert qc.line_count("a\nb\nc\n") == 3
        assert qc.line_count("") == 0

    def test_file_length_violation_only_over_limit(self):
        under = "x = 1\n" * qc.MAX_FILE_LINES
        over = "x = 1\n" * (qc.MAX_FILE_LINES + 1)
    ...
    def test_file_length_exempt_data_module_is_not_flagged(self):
        over = "x = 1\n" * (qc.MAX_FILE_LINES + 1)
    ...
    def test_file_length_exempt_matches_nested_and_absolute_paths(self):
        exempt = next(iter(qc.FILE_LENGTH_EXEMPT))
    ...
    def test_file_length_exempt_set_is_exactly_the_documented_data_modules(self):
    ...
    def test_each_exempt_module_is_a_real_over_cap_data_file(self):
    ...
    def test_coupling_violation_fires_when_a_module_rises_above_its_floor(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
    ...
    def test_lcom4_one_when_methods_share_an_attribute(self):
        cls = ast.parse(
            "class C:\n    def a(self):\n        self.x = 1\n    def b(self):\n        return self.x\n"
        ).body[0]
        assert qc.lcom4(cls) == 1

    def test_lcom4_one_when_a_method_calls_another(self):
        cls = ast.parse(
            "class C:\n    def a(self):\n        return self.b()\n    def b(self):\n        return 1\n"
        ).body[0]
        assert qc.lcom4(cls) == 1

    def test_lcom4_two_when_methods_are_disjoint(self):
        cls = ast.parse(
            "class C:\n    def a(self):\n        return self.x\n    def b(self):\n        return self.y\n"
        ).body[0]
        assert qc.lcom4(cls) == 2

    def test_lcom4_one_for_single_method_class(self):
        cls = ast.parse("class C:\n    def a(self):\n        return self.x\n").body[0]
        assert qc.lcom4(cls) == 1

    def test_cohesion_violation_fires_above_threshold(self, tmp_path, monkeypatch):
    ...
    def test_depth_of_inheritance_follows_internal_bases(self):
    ...
    def test_number_of_children_counts_internal_subclasses(self):
        children = qc.number_of_children({"A": [], "B": ["A"], "C": ["A"], "D": ["B"]})
        assert children["A"] == 2
        assert children["B"] == 1

    def test_inheritance_violation_flags_deep_dit(self, tmp_path, monkeypatch):
    ...
    def test_inheritance_violation_flags_high_noc(self, tmp_path, monkeypatch):
    ...
    def test_live_tree_passes_the_architectural_guard_rails(self):
        files = qc.python_files(qc._DEFAULT_ROOTS)
        assert qc.cohesion_violations(files) == []
        assert qc.inheritance_violations(files) == []

    def test_service_barrel_is_invisible_to_transitive_coupling(self):
    ...
    def test_live_tree_sits_exactly_on_its_ratchet_floors(self):
        files = qc.python_files(qc._DEFAULT_ROOTS)
        assert qc.collect(qc._DEFAULT_ROOTS) == []
        assert qc.compute_baseline(files) == qc.load_baseline()

    def test_param_reach_live_total_is_pinned_to_the_baseline(self):
    ...
    def test_param_reach_store_dispatch_dominates_the_sibling_handles(self):
        trees = qc.parse_files(qc.python_files(qc._DEFAULT_ROOTS))
        counts = qc.param_reach_by_handle(trees)
...
class TestQualityChecks2:
    def test_spreadsheet_shared_state_within_cap(self):
        files = qc.python_files(qc._DEFAULT_ROOTS)
        assert qc.spreadsheet_shared_state_violations(files) == []

    def test_spreadsheet_shared_state_fires_when_over_cap(self, monkeypatch):
    ...
    def test_reach_through_counts_every_injected_handle_not_just_page(self, tmp_path):
    ...
    def test_collect_and_main_over_a_tree(self, tmp_path, capsys):
        bad = tmp_path / "pkg"
        bad.mkdir()
    ...
    def test_main_clean_tree_returns_zero(self, tmp_path):
