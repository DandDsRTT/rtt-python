...
class TestCoverageFloor:
    def test_file_percents_keeps_only_logic_tier_files(self):
        data = report(
            {
    ...
    def test_floors_at_baseline_pass(self):
        percents = {"rtt/library/a.py": 88.0, "rtt/app/service/b.py": 90.5}
        assert cf.floor_violations(percents, dict(percents)) == []

    def test_a_file_dropping_below_its_floor_fails_only_for_that_file(self):
        percents = {"rtt/library/a.py": 88.0, "rtt/app/service/b.py": 90.5}
    ...
    def test_improvement_above_floor_passes(self):
        assert cf.floor_violations({"rtt/library/a.py": 95.0}, {"rtt/library/a.py": 88.0}) == []

    def test_a_removed_baselined_file_is_skipped_not_flagged(self):
        assert cf.floor_violations({"rtt/library/a.py": 90.0}, {"rtt/library/gone.py": 99.0}) == []

    def test_main_update_baseline_then_passes(self, tmp_path, monkeypatch):
    ...
    def test_main_returns_nonzero_when_a_file_regresses(self, tmp_path, monkeypatch):
        report_path = tmp_path / "coverage.json"
