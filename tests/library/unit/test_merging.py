...
class TestMerging:
    ...
    def test_map_merge_to_meantone(self, factors):
    ...
    def test_comma_merge_to_et7(self, factors):
        assert comma_merge(*factors) == ET7_C

    def test_map_merge_three_temperaments(self):
        assert map_merge(ET7D_M7, ET12_M7, ET22_M7) == MARVEL

    def test_comma_merge_three_temperaments(self):
    ...
    def test_merge_through_ebk(self, merge, inputs, expected):
        result = merge(*(parse_temperament_data(s) for s in inputs))
        assert to_ebk(result) == expected

    def test_map_merge_nonstandard_bases(self):
    ...
    def test_map_merge_mixed_variance_nonstandard(self):
    ...
    def test_comma_merge_nonstandard_to_standard(self):
        t1 = Temperament(((4, -4, 1),), COL)
        t2 = Temperament(((6, -1, -1),), COL, (2, 9, 7))
        assert comma_merge(t1, t2) == Temperament(((4, -4, 1, 0), (-6, 2, 0, 1)), COL)

    def test_comma_merge_nonstandard_result(self):
