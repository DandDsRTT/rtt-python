...
NESTED_MINIMAX_CASES = [
    ("unityWeight", 1, 0, (600.000, 108.128)),
    ("simplicityWeight", 1, 0, (596.502, 106.708)),
    ("complexityWeight", 1, 0, (600.581, 107.714)),
]
...
SRUTAL_DAMAGE = [
    ...
    (1, "copfr-S-damage", (599.052, 1901.955)),
...
class TestTuning:
    ...
    def test_optimize_generator_tuning_map_nested_minimax(self, slope, norm_power, log_prime_power, expected):
    ...
    def test_optimize_generator_tuning_map_norm_power_continuum(self, norm_power, expected):
        t = parse_temperament_data(MEANTONE)
        spec = TuningSchemeSpec(
            optimization_power=inf,
            target_intervals=SIX_TILT,
    ...
    def test_optimize_generator_tuning_map_power_continuum(self, power, expected):
    ...
    def test_optimize_generator_tuning_map_systematic_name(self, scheme_name, expected):
        t = parse_temperament_data(PAJARA)
    ...
    def test_optimize_generator_tuning_map_damage_name(self, power, damage_name, expected):
    ...
    def test_optimize_generator_tuning_map_complexity_name(self, power, slope, complexity_name, expected):
        t = parse_temperament_data(BLACKWOOD)
    ...
    def test_complexity_name_traits_surfaces_held_separately(self):
        traits, held = complexity_name_traits("lols-complexity")
        assert "held_intervals" not in traits
        assert held == "octave"
        plain_traits, plain_held = complexity_name_traits("copfr-complexity")
        assert "held_intervals" not in plain_traits
        assert plain_held is None

    @pytest.mark.parametrize("held", ["octave", "2", "2/1", "{2}", "{2/1}"])
    def test_held_octave_synonyms_dict(self, held):
        t = parse_temperament_data(MEANTONE)
        spec = TuningSchemeSpec(
    ...
    @pytest.mark.parametrize("held", ["octave", "2", "2/1", "{2}", "{2/1}"])
    def test_held_octave_synonyms_name(self, held):
        t = parse_temperament_data(MEANTONE)
    ...
    def test_held_two_intervals(self):
        t = parse_temperament_data(MEANTONE)
        spec = TuningSchemeSpec(
            optimization_power=1,
            target_intervals=FIVE_OLD,
    ...
    def test_held_interval_minimax(self):
        t = parse_temperament_data(MEANTONE)
        spec = TuningSchemeSpec(
            optimization_power=inf,
            target_intervals=FIVE_OLD,
    ...
    def test_held_intervals_determine_tuning(self):
        t = parse_temperament_data(MEANTONE)
    ...
    def test_held_tilt(self, scheme, expected):
        t = parse_temperament_data(MEANTONE)
        assert optimize_generator_tuning_map(t, scheme) == pytest.approx(expected, abs=TOL)

    @pytest.mark.parametrize("ebk, expected", HELD_OCTAVE_OLD_GENERATORS)
    def test_held_octave_old_generators(self, ebk, expected):
    ...
    def test_held_octave_old_tuning_map(self):
    ...
    def test_get_just_tuning_map_standard(self):
    ...
    def test_get_just_tuning_map_nonstandard_basis(self):
    ...
    def test_get_complexity(self, norm_power, log_prime_power, expected):
    ...
    def test_get_dual_power(self, power, expected):
