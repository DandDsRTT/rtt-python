...
ERROR_PAIRS = [
    (SEPTIMAL_MEANTONE_M, SEPTIMAL_BLACKWOOD_M),
...
class TestAddition:
    ...
    def test_grade_one_addition(self, a, b, op, expected):
    ...
    def test_addition_through_ebk(self, ebk_a, ebk_b, op, expected):
    ...
    def test_linearly_dependent_addition(self, a, b, op, expected):
        assert op(a, b) == expected
    ...
    def test_addition_examples(self, a, b, op, expected):
        assert op(a, b) == expected

    @pytest.mark.parametrize("t", [MEANTONE_M, MEANTONE_C, ET7_M, dual(ET7_M)])
    def test_self_sum_returns_self(self, t):
        assert sum_(t, t) == t

    @pytest.mark.parametrize("t", [MEANTONE_M, MEANTONE_C, ET7_M, dual(ET7_M)])
    def test_self_diff_errors(self, t):
        with pytest.raises(ValueError):
            diff_(t, t)

    def test_addition_mixed_variance(self):
    ...
    def test_addition_is_variance_consistent(self):
    ...
    def test_addition_errors(self, op, a, b):
        with pytest.raises(ValueError):
            op(a, b)

    @pytest.mark.parametrize("op", [sum_, diff_])
    @pytest.mark.parametrize("a, b", DUALITY_SWAP_PAIRS)
    def test_addition_duality_consistent_regressions(self, op, a, b):
        assert op(a, b) == dual(op(dual(a), dual(b)))

    def test_compton_meantone_sum_is_schisma_on_both_sides(self):
        c1, c2 = Temperament(((-19, 12, 0),), COL), Temperament(((4, -4, 1),), COL)
        m1, m2 = dual(c1), dual(c2)
        assert sum_(c1, c2) == Temperament(((-15, 8, 1),), COL)
    ...
    def test_addition_duality_consistent_fuzz(self):
    ...
    def test_addition_preserves_domain_basis(self):
    ...
    def test_find_modular_solution_is_bounded_and_correct(self):
        rng = random.Random(7)
        modulus = 111
    ...
    def test_find_modular_solution_raises_when_no_solution_exists(self, ldb, base, modulus):
        with pytest.raises(ValueError, match="no modular solution"):
            _find_modular_solution(ldb, base, modulus)

    def test_heavy_13_limit_sum_is_fast_and_consistent(self):
