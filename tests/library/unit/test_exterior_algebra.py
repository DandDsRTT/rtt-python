...
class TestExteriorAlgebra:
    def test_ea_dimensions(self):
        assert ea_get_d(MEANTONE_MM) == 3
        assert ea_get_r(MEANTONE_MM) == 2
        assert ea_get_n(MEANTONE_MM) == 1

    def test_ea_nullity_of_a_contravariant_multivector_is_its_grade(self):
        assert ea_get_n(MEANTONE_MC) == 1, "for a comma (COL) multivector the nullity is read straight off the grade, not d - grade: # meantone's single comma 81/80 is grade 1, so nullity 1"

    @pytest.mark.parametrize("accessor", [ea_get_d, ea_get_r, ea_get_n])
    def test_ea_dimensions_undefined_for_a_nondecomposable_multivector(self, accessor):
        with pytest.raises(ValueError):
            accessor(NONDECOMPOSABLE)

    def test_ea_accessors(self):
    ...
    def test_is_nondecomposable(self):
    ...
    def test_ea_indices(self, d, grade, expected):
    ...
    def test_ea_canonical_form(self, u, expected):
        assert ea_canonical_form(u) == expected

    def test_ea_canonical_form_nondecomposable_errors(self):
        with pytest.raises(ValueError):
    ...
    def test_multivector_to_matrix(self, u, expected):
    ...
    def test_multivector_to_matrix_errors(self, u):
        with pytest.raises(ValueError):
    ...
    def test_ea_dual(self, u, expected):
        assert ea_dual(u) == expected

    def test_ea_dual_nondecomposable_errors(self):
        with pytest.raises(ValueError):
    ...
    def test_ea_dual_is_an_involution(self, t):
    ...
    def test_ea_products(self, op, u1, u2, expected):
        assert op(u1, u2) == expected

    @pytest.mark.parametrize(
        "op, u1, u2",
        [
            (progressive_product, D3G2CONTRA1, D3G2CONTRA2),
            (progressive_product, D3G2CO1, D3G2CO2),
            (regressive_product, D3G1CONTRA1, D3G1CONTRA2),
            ...
            (progressive_product, D5G1CONTRA, D5G3CO),
            (regressive_product, D5G1CONTRA, D5G3CO),
            (right_interior_product, D5G2CO1, D5G2CO2),
            (left_interior_product, D5G2CO1, D5G2CO2),
            (interior_product, D5G2CO1, D5G2CO2),
            (right_interior_product, D3G2CONTRA1, D3G2CONTRA2),
            ...
            (progressive_product, matrix_to_multivector(MEANTONE_M),
             matrix_to_multivector(Temperament(((1, 2, 3), (0, 3, 5)), ROW))),
            (right_interior_product, D3G1CONTRA1, D3G1CO2),
    ...
    def test_ea_product_errors(self, op, u1, u2):
        with pytest.raises(ValueError):
            op(u1, u2)
    ...
    def test_matrix_to_multivector(self, t, expected):
    ...
    def test_ea_addition(self, op, u1, u2, expected):
        assert op(u1, u2) == expected
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (ea_sum, _ET7_MM, _MEANTONE_MM),
            (ea_sum, _ET7_MC, _MEANTONE_MC),
            (ea_sum, _ET7_MM, _ET12_MM),
    ...
    def test_ea_addition_errors(self, op, u1, u2):
        with pytest.raises(ValueError):
            op(u1, u2)

    def test_u_to_tensor(self):
    ...
    def test_matrix_to_multivector_dependent_leading_rows(self, matrix, variance, expected):
    ...
    def test_ea_diff_with_itself_raises(self, u):
        with pytest.raises(ValueError, match="cannot diff a temperament with itself"):
            ea_diff(u, u)

    def test_ea_addition_non_addable_message(self):
    ...
    def test_progressive_product_dimensionality_mismatch(self, u1, u2):
